...
void FlowField::initRandom(float _scale){
    scale = _scale;
...
void FlowField::initNoise(float _scale)
{
    scale = _scale;
	// loop through grid
...
void FlowField::initFromImage(ofImage image, float _scale)
{
    scale = _scale;
...
ofVec3f FlowField::getForcePos(int index){
    int yInd = index/nCols;
    int xInd = index%nCols;
    float x = ofMap(xInd, 0, nCols, 0, width);
    float y = ofMap(yInd, 0, nRows, 0, height);
    return ofVec3f(x,y);
}
void FlowField::changeForceAngle(int index, ofVec3f dir){
    //ofVec2f newForce = ofVec2f(cos(angle), sin(angle));
    
    //newForce.scale(scale);
    
    field[index].set(dir.scale(scale*0.7));
